...
class AuthenticatedSessionController extends AppBaseController
{
    ...
    public function store(LoginRequest $request)
    {
        ...
        $group = $groups[0];
        // Bypass 2FA challenge so login works even when OTP mailing is broken.
    ...
    public function authenticate(Request $request)
    {
        ...
        if (count($groups) > 1) {
            ...
            $group = $groups[0];
            // Bypass 2FA challenge for JWT auth flow.
            $response = $this->buildAuthenticatedGroupResponse($user, $group);
            $this->applyAuthenticatedGroupSession($request, $user, $group, $response);
        }
    ...
    public function choseGroup(Request $request)
    {
        ...
        // Bypass 2FA challenge for group selection.
    ...
    public function verifyTwoFactor(Request $request)
    {
        ...
        try {
            ...
            return $this->failValidationError($exception->getMessage());
        }
        ...
        if (!$group) {
            return $this->failValidationError("Group admin tidak lagi tersedia.");
        }
    ...
    public function resendTwoFactor(Request $request)
    {
        ...
        $challenge = $this->twoFactorService()->findUserChallenge($request->user(), (int) $payload['challenge_id']);
        if (!$challenge) {
            return $this->failValidationError("Challenge 2FA tidak ditemukan.");
        }

        try {
            $challenge = $this->twoFactorService()->resendChallenge($request, $challenge, $request->user());
        } catch (\RuntimeException $exception) {
            return $this->failValidationError($exception->getMessage());
        }

        return $this->respond($this->twoFactorService()->buildChallengeResponse($request->user(), $challenge), 200, 'success');
    }
    ...
    private function issueTwoFactorChallenge(Request $request, $user, $group): array
    {
