...
export default ({ ...props }) => {
  ...
  const handlepreview = async () => {
    ...
    const thead = document.getElementById("table_kri_head");
    const tbody = document.querySelector("#table_kri>tbody");
    const trtd_all = document.querySelectorAll("#table_kri>tbody>tr");
    const table_kri_chart_all = document.querySelectorAll(".table_kri_chart");
    const table_kri_chart_arr = Array.from(table_kri_chart_all);

    const trtd_arr = Array.from(trtd_all);

    // console.log('table_kri_chart_all')
    // console.log(table_kri_chart_all)
    // console.log(table_kri_chart_arr)

    console.log("trtd_arr");
    // console.log(trtd_all)
    console.log(trtd_arr);

    let tablehtml = "";

    tablehtml +=
      '<table id="table_kri" className="w-full table table-auto border-collapse border">';
    tablehtml += thead.outerHTML;
    tablehtml += "<tbody>";

    for (let y = 0; y < trtd_arr.length; y++) {
      let x = trtd_arr[y];
      if (true) {
        // if (x.className && x.className.includes('is_kuantitatif')) {
        if (true) {
          for (let i = 0; i < table_kri_chart_all.length; i++) {
            if (i == y) {
              const img = await handleHtmlToImage(
                table_kri_chart_all[i],
                "toPng"
              );
              x.lastElementChild.innerHTML = img.img.outerHTML;
              tablehtml += x.outerHTML;
              break;
            }
          }
        } else {
      ...
      >
        <Modal.Header closeButton={true}>
          ...
          <h5 className="ml-1 mt-1">
            {is_loading ? "Sedang loading mohon tunggu..." : props.name}
          </h5>
        </Modal.Header>
        <Modal.Body>
